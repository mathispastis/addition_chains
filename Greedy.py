...
def greedy(input_sequence):
    """
    Main execution of the Greedy Smallest Grammar Problem algorithm.
    Stitches together the sieve, the counter, and the replacer.
    """
    ...
    while True:
        ...
        # If no better rules are found , we are done
        if max_savings <= 0 or best_substring is None:
            break

        new_rule_name = f'A{rule_counter}'

        # We replace in existing rules FIRST to avoid cycles
        # (A1 cannot replace the tokens inside the definition of A1)
        replace_tokens(grammar, best_substring, new_rule_name)

        # Now we add the definition for the new rule to our grammar
...
#result = greedy('abaaaabaaabbbbbba')
#print(result)